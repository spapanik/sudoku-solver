...
from sudoku_solver.constants import SudokuConstants


class SudokuPuzzle:
    __slots__ = ("constants", "cells", "candidates")
    top = "┌─────┬─────┬─────┐"
    mid = "├─────┼─────┼─────┤"
    bottom = "└─────┴─────┴─────┘"
    ...
    def _hidden_single(self) -> bool:
        """
        If a digit only appears once in a row, column, or box, set it and remove it from the candidates.
        """
        advanced = False
        for index in set(self.candidates.keys()):
            blocks = [
    ...
    def _naked_pair_for_group(self, group: frozenset[int]) -> bool:
        ...
        for index_1, index_2 in combinations(indices, 2):
            ...
            if len(digits) != 2 or digits != self.candidates[index_2]:
                continue
    ...
    def _naked_pair(self) -> bool:
        """
        If two cells in a group have the same two candidates, remove those candidates from the other cells in the group.
        """
        advanced = False
        for group in chain(
    ...
    def _hidden_pair_for_group(self, group: frozenset[int]) -> bool:
        ...
        for index_1, index_2 in combinations(indices, 2):
            if len(self.candidates[index_1] | self.candidates[index_2]) == 2:
                continue
    ...
    def _hidden_pair(self) -> bool:
        """
        If two digits only appear in two cells in a group, remove all other candidates from those cells.
        """
        advanced = False
        for group in chain(
            self.constants.rows, self.constants.columns, self.constants.boxes
    ...
    def _locked_candidates(self) -> bool:
        """
        If a digit only appears in a box in a row or column,
        or in a row or column in a box,
        remove it from the rest of the row or column.
        """
        advanced = False
