...
class SudokuPuzzle:
    __slots__ = ("candidates", "cells", "constants", "given", "last_step")
    top = "┌─────┬─────┬─────┐"
    mid = "├─────┼─────┼─────┤"
    bottom = "└─────┴─────┴─────┘"
    ...
    def __init__(self, string: str) -> None:
        ...
        self.given = frozenset(
            index for index, cell in enumerate(self.cells) if cell != "."
        )
        self.last_step: set[int] = set()
    ...
    def format(
        self, *, pretty: bool, highlight: AbstractSet[int] = frozenset()
    ) -> SGROutput:
        """Format the puzzle for printing, with the highlighted cells in green."""
        fragments = []
        plain = ""
        for text, index in self._tokens(pretty=pretty):
            if index in highlight:
                if plain:
                    fragments.append(SGRString(plain))
                    plain = ""
                fragments.append(SGRString(text, params=[SGRCodes.GREEN]))
            else:
                plain += text
        if plain:
            fragments.append(SGRString(plain))
        return SGROutput(fragments)
    ...
    def step(self) -> str | None:
        """Apply the first technique that makes progress.

        Return the name of the technique used, or None if the puzzle
        cannot be advanced any further.
        """
        self.last_step = set()
        techniques: tuple[tuple[str, Callable[[], bool]], ...] = (
            ("naked single", self._naked_single),
            ("hidden single", self._hidden_single),
            ("naked pair", self._naked_pair),
            ("hidden pair", self._hidden_pair),
            ("locked candidates", self._locked_candidates),
            ("x-wing", self._x_wing),
        )
    ...
    @property
    def calculated(self) -> set[int]:
        return {
            index
            for index, cell in enumerate(self.cells)
            if cell != "." and index not in self.given
        }

    @property
    def matrix(self) -> str:
        return "".join(text for text, _ in self._tokens(pretty=True))
    ...
    def _tokens(self, *, pretty: bool) -> Iterator[tuple[str, int | None]]:
        """Yield the fragments of the formatted puzzle, with their cell index.

        Decorative fragments (borders, spacing) have an index of None.
        """
        if not pretty:
            for index in range(81):
                yield self.cells[index], index
            return
        yield f"{self.top}\n", None
        for row in range(9):
            if row in {3, 6}:
            ...
            for column in range(9):
                index = 9 * row + column
                yield self.cells[index], index
                if column == 8:  # noqa: PLR2004
                    yield "│\n", None
                elif column % 3 == 2:  # noqa: PLR2004
                    yield "│", None
                else:
                    yield " ", None
        yield f"{self.bottom}\n", None
